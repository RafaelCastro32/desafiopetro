'''
Generates a labeled test dataset and creates two text files:
1) test_labels.txt with the image names and correct labels
2) test_files.txt with the image names
This script also generates files with the bounding boxes in case
someone wants to check object detection metrics such as IoU.
Aldebaro. June 2022.
'''
...
output_file_name = 'test_labels.txt'
images_file_name = 'test_files.txt'
path = "../../original_images_data/task3_digits/small_images" #input folder
output_folder = '../../yolo_images_data/test_dataset/' #end with /
should_show_images = True #use True to plot images, and False to run faster
...
epoch=0 #keep it zero
num_of_written_images = 0
with open(output_file_name, 'w') as output_file:
    #for epoch in range(num_epochs): #a complete sweep over all digits
    with open(images_file_name, 'w') as images_file:
        for image_counter in range(N): #go over all files
            n = random.randint(0,len(all_digit_files_list)-1)
            ...
            if True:
                ...
                #get the two digits of interest:
                upper_digit = int(five_classes[0]*10+five_classes[1])
                print('upper_digit=',upper_digit)
                lower_digit = int(10*five_classes[3]+five_classes[4])
                if five_classes[2]==1:
                    lower_digit *= -1
                print('lower_digit=',lower_digit)
                ...
                output_file.write('image_' + str(image_counter+1) + ' ' + os.path.basename(output_file_name) + '\n')
                output_file.write(str(upper_digit) + '\n')
                output_file.write(str(lower_digit) + '\n')

                #relative path:
                images_file.write(output_file_name + '\n')
                #full path:
                #images_file.write(os.path.abspath(output_file_name) + '\n')
